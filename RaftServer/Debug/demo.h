...
/*  
    tip:  
    electdue是一个位于主线程loop的计时器,负责本节点的选举计时,Heartbeat则是位于
    每个RaftNode所属的loop的计时器,负责RPC计时
*/

/* 
    tip:
    fastNet中对新建连接的处理,是在主线程new Connection,然后再将shared_ptr指针交给
    分配给这个连接的线程,这是unsafe的,原因是主线程和conn所属线程都访问conn, 主线程中针
    对conn调用的各种set函数理论上应该加所属线程的锁,但这会引起所属线程访问conn也需要加锁,
    加锁得不偿失,本程序中将conn的构建移到了conn被分配的线程上,使conn的访问仅在其所属线程进行,
    防止无意义加锁
*/

/*
    tip:
    在主线程调用的call函数是让RaftNode所属的线程执行callInloop,由于主线程调用call
    将callInloop送入node所属线程的任务队列后就返回了，即使在调用call之前RaftNode被
    shared_ptr保护,但在node所属线程中执行callInloop时没有shared_ptr保护，这就导致
    callInloop执行过程有被析构的危险,解决方案是将RaftNode的shared_ptr放入callInloop
    作为参数保护其执行
*/

/*
    tip:
    vector<shared_ptr<>>尽量避免使用push_back添加元素,会造成不正确的析构触发某些类的特殊的析构函数
    造成难以解决的bug.
*/
...
class RaftServer
{
    ...
    /*  updateCommit时调用 */
    void clientCallback(weak_ptr<Connection> conn, int logId, string flag, string oper, string process)
    {
        if (process == "commit")
        {
    ...
    Mutex mutex_;
    unique_ptr< ThreadLoop > worker_;
    unique_ptr< map<string, string> > mapkv_;
    int curLogId_;
    int lastApplied;
    /* 
