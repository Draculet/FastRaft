    ...
    Log(int term, int index,
        int logId, string oper,
             string key, string value,
                function<void(string)> callback = function<void(string)>())
        :term_(term),
        index_(index),
        logId_(logId),
...
class ClientRes
{
    ...
    string toString()
    {
        ...
        int leaderId = htons(leaderId_);
        memcpy(buf + 8, &leaderId, 4);
        memcpy(buf + 12, flag_.c_str(), 4);
        int respsize = htons(resp_.size());
...
class RaftServer
{
    ...
    void workerCallback()
    {
        ...
        vector<Log> logs;
        {
            ...
            printf("_______commitIndex: %d________\n", commitIndex);
            curLogId = curLogId_;
        ...
        {
            printf("continue2\n");
            if (log.getOper() == "get" || log.getLogId() == curLogId_ + 1)
            {
                printf("*******cur LogId %d*********\n", curLogId_);
                applyLog(log);
                if (log.getLogId() == curLogId_ + 1)
                {
                    curLogId_++;
                    printf("*******update cur LogId %d*********\n", curLogId_);
                }
            }
        }
        printf("continue4\n");
        ...
        while (true)
        {
            ...
            else if (type == "CliReq")
            {
                ...
                if (role_ == leader)
                {
                    ...
                    printf("%d %s %s %s %s", logId, oper.c_str(), key.c_str(), value.c_str(), flag.c_str());
                    if (req.getOper() == "set" || req.getOper() == "del" || req.getOper() == "get")
                    {
                        printf("\n\n\n_______________Leader InsertLog_______________\n\n\n");
                        statMachine_->insertLog(nextIndex, Log(
                                term, nextIndex, 
                                    logId, oper,
                                        key, value,
                ...
                else /* role != leader */ 
                {
                    ...
                    printf("_____________ClientReq Failed__logId:%d___voteFor:%d_________", logId, statMachine_->getVoteFor());
                    conn->send(res.toString());
                }
